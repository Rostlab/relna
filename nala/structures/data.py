...
class Part:
    ...
    def __init__(self, text, is_abstract=True):
        ...
        """
        self.tokens = []
        """a list of tokens derived from the sentences"""
        self.annotations = []
    ...
    def get_entities_in_sentence(self, sentence_id, entity_classId=None):
        """
        ...
        for annotation in self.annotations:
            if (entity_classId):
                ...
                if start <= annotation.offset < end:
                    entities.append(annotation)
        return entities

    def percolate_tokens_to_entities(self):
        """
...
class Edge:
    ...
    def get_edge_without_offset(self):
        return (self.entity1.text, self.relation_type, self.entity2.text)
...
class Token:
    ...
    def get_entity(self, part):
        """
        there are two cases:
        * if entity is a subset of the token
        * if token is a subset of the entity
        """
        for entity in part.annotations:
...
class Entity:
    ...
    """
    def __init__(self, class_id, offset, text, confidence=1, tokens=[]):

    ...
    def generate(self, dataset, feature_set, is_training_mode):
        for edge in dataset.edges():
            ...
            for i in range(head1.features['id'], head2.features['id']):
                token1 = edge.part.sentences[edge.same_sentence_id][i]
                token2 = edge.part.sentences[edge.same_sentence_id][i+1]
        ...
        for edge in dataset.edges():
            ...
            if head2.features['id']-head1.features['id']==2:
                for i in range(head1.features['id'], head2.features['id']):
                    token1 = edge.part.sentences[edge.same_sentence_id][i]
                    token2 = edge.part.sentences[edge.same_sentence_id][i+1]
                    token3 = edge.part.sentences[edge.same_sentence_id][i+2]
